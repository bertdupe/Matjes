...
subroutine restart_read_spin(fname,spin)
implicit none
character(len=*), intent(in) :: fname
real(kind=8), intent(inout) :: spin(:,:,:,:,:)
! internal
integer :: j,i_x,i_y,i_z,i_m,shape_spin(5)
integer :: io
  ...
  do i_z=1,shape_spin(4)
    do i_y=1,shape_spin(3)
      do i_x=1,shape_spin(2)
         ...
         read(io,*) (spin(j,i_x,i_y,i_z,i_m),j=1,shape_spin(1))
...
subroutine restart_write_spin(fname,spin)
implicit none
character(len=*), intent(in) :: fname
...
subroutine restart_read_replicas(fname,replicas)
implicit none
character(len=*), intent(in) :: fname
real(kind=8), intent(inout) :: replicas(:,:,:)
! internal
integer :: i,j,k,shape_rep(3)
integer :: io

shape_rep=shape(replicas)
...
do i=1,shape_rep(3)
  ...
  do j=1,shape_rep(2)
    ...
    read(io,*) (replicas(k,j,i),k=1,shape_rep(1))
...
subroutine restart_write_replicas(fname,replicas)
implicit none
character(len=*), intent(in) :: fname
...
do i=1,shape_rep(3)
   ...
   do j=1,shape_rep(2)
      ...
      write(io,'(7E20.10E3)') (replicas(k,j,i),k=1,shape_rep(1))
