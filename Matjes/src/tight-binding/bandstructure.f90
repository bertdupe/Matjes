module m_bandstructure
    ...
    complex(kind=8), allocatable, dimension(:) :: all_vectors ! will contain all the order parameters in one line
    ...
    complex(kind=8), allocatable, dimension(:,:) :: all_E
    ...
    contains
        subroutine set_E_bandstructure(dim_mode,pos)
            ...
            all_vectors=cmplx(0.0d0, kind=8)
            all_E=cmplx(0.0d0, kind=8)
            all_positions=0.0d0

            do i=1,N
                i_vois=energy%line(i,1) !all neighbours of site 1
                all_E(:,(i-1)*dim_mode+1:i*dim_mode)=transpose(cmplx(energy%value(i,1)%order_op(1)%Op_loc, kind=8))
        ...
        ! Function computing the total energy for each kpoint in the mesh
        subroutine calculate_dispersion(all_mode, dispersion, dim_mode, nb_kpoint, N_cell)
            ...
            complex(kind=8), intent(inout) :: dispersion(:)
            ...
            do i=1, nb_kpoint
                do j=1, N_cell !loop to get the sites
                    do k=1, N !loop to get the neighbours of site j
                        ...
                        all_vectors((k-1)*dim_mode+1:k*dim_mode) = cmplx(all_mode(i_vois)%w, kind=8) !all_mode is the vector of ordre parameter for each site. We put everything in a big vector
                    enddo
                    dispersion(i) = dispersion(i) + get_FFT(all_E, all_positions, all_vectors, cmplx(all_mode(j)%w, kind=8), i, dim_mode*N, dim_mode, -1.0d0) !dispersion(i)=dispersion(i)+... accounts for the sum over r
                enddo
            enddo
