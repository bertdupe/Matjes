module m_bandstructure
   ...
   contains
       ...
       subroutine calculate_dispersion(all_mode, dispersion, dim_mode, nb_kpoint)
           ...
           ! Internal variable
           integer :: i, j, k, N, i_vois, N_cell

           ! Gives the number of sites in the lattice
           N=size(energy%line(:,1))
           N_cell=size(all_mode)

           do i=1, nb_kpoint
               ! sum on the sites
               do j=1, N_cell

                   ! sum on the neighbors
                   do k=1, N
...
!                       get_E_k_local(all_energy, pos, all_vectors, X_site, pos_k, size_all_vectors, dim_mode, sense)
                    dispersion(i) = dispersion(i) + get_FFT(all_E, all_positions, all_vectors, cmplx(all_mode(j)%w,kind=16), i, dim_mode*N, dim_mode, -1.0d0)
               enddo
           enddo
