...
! 1D case
       if (size(N).eq.1) then
        ...
        do while (int(u).le.k)
         j=j+1
         res(j)= &
          norm(r(1,:)*(u+motif%pos(1,1))+r(2,:)*motif%pos(1,2)+r(2,:)*motif%pos(1,3))
         u=u+1.0d0
        enddo
!-----------------------------------------
! 2D case
       elseif (size(N).eq.2) then
        ...
        v=0.0d0
         do while (int(v).le.k)
          j=j+1
          test_vec=r(1,:)*(u+motif%pos(1,1))+r(2,:)*(v+motif%pos(1,2))+r(3,:)*motif%pos(1,3)
          res(j)=norm(test_vec)
          v=v+1.0d0
         enddo
        u=u+1.0d0
        ...
        v=0.0d0
         ...
         w=0.0d0
          do while (int(w).le.k)
           j=j+1
           res(j)= &
            norm(r(1,:)*(u+motif%pos(1,1))+r(2,:)*(v+motif%pos(1,2))+r(3,:)*(w+motif%pos(1,3)))
           w=w+1.0d0
          enddo
          v=v+1.0d0
         enddo
...
! 1D case
       if (size(N).eq.1) then
        ...
        do while (int(u).le.k)
         ...
         res(j,1)= &
          norm(r(1,:)*(u+motif%pos(1,1))+r(2,:)*motif%pos(1,2)+r(2,:)*motif%pos(1,3))
         res(j,2:4)=r(1,:)*(u+motif%pos(1,1))+r(2,:)*motif%pos(1,2)+r(2,:)*motif%pos(1,3)
         u=u+1.0d0
        enddo
!-----------------------------------------
! 2D case
        ...
        v=0.0d0
         ...
         w=0.0d0
          do while (int(w).le.k)
           ...
           res(j,1)= &
            norm(r(1,:)*(u+motif%pos(1,1))+r(2,:)*(v+motif%pos(1,2))+r(3,:)*(w+motif%pos(1,3)))
           res(j,2:4)=r(1,:)*(u+motif%pos(1,1))+r(2,:)*(v+motif%pos(1,2))+r(3,:)*(w+motif%pos(1,3))
           w=w+1.0d0
          enddo
          v=v+1.0d0
         enddo
