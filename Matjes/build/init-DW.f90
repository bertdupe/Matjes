...
subroutine init_DW(my_lattice,my_motif)
type (lattice), intent(inout) :: my_lattice
type (cell), intent(in) :: my_motif
! internal variables
integer :: i_z,i_y,i_x,i_m,Nx,Ny,Nz,size_mag,dw_position,i_w
real(kind=8) :: alpha
   ...
   do i_w=-4,4
      ...
      if ( i_x+i_w == dw_position ) then
         do i_y=1,Ny
           ...
           my_lattice%l_modes(i_x,i_y,i_z,i_m)%w(1)=sin(alpha)
           my_lattice%l_modes(i_x,i_y,i_z,i_m)%w(2)=0.0d0
           my_lattice%l_modes(i_x,i_y,i_z,i_m)%w(3)=-1.0d0*cos(alpha)
      ...
      elseif ( i_x+i_w .gt. dw_position ) then
         do i_y=1,Ny
           ...
           my_lattice%l_modes(i_x,i_y,i_z,i_m)%w(1:2)=0.0d0
           my_lattice%l_modes(i_x,i_y,i_z,i_m)%w(3)=-1.0d0
