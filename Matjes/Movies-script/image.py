...
top_panel_plot = "Total topological charge"		         #other possibilities: "Magnetic field" or "Torque in eV" -> compare with top_panel function in line 90 etc
size = 0.2					 # 1 equals to 100% and 0 to 0%
scale = 0.08					 # colorbar scale: requires cb_max = False otherwise this setting will be ignored
cb_max = False					 # activates/deactivates cb_max-loop on True/False




#image						#shows image directly and/or saves the image in the same directory
show = True
save = False
dpi = 100					#resolution
...
def get_shape(filename):			# for lattice.in
    f = open(filename, 'r')
    for line in f:
        if "Nsize" in line:
            a = line.split()
...
n = input("Filenumber? (Allowed natural numbers lay between 1 and "+str(duration)+") Type: ")
...
if cb_max == True:
    ...
    for j in range(duration):
        x, y, density = load_topo("topomap"+str(j+1)+".dat", left_panel_plot, shape)
        cb_max.append(np.max(density))
        print str(j+1)+" of "+str(duration)+" files checked."
...
if save == True:
    plt.savefig('spin_'+str(n)+'.png', dpi=dpi)
if show == True:
    plt.show()
