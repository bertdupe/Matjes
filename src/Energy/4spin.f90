...
subroutine read_sp4_input(io_param,fname,io)
    use m_io_read_util, only: set_pos_entry, check_further_entry
    use, intrinsic :: iso_fortran_env, only : output_unit,error_unit
    integer,intent(in)              :: io_param
    character(len=*), intent(in)    :: fname
    type(io_H_sp4),intent(out)      :: io
    character(len=*),parameter      :: var_name="M_4spin"
...
subroutine get_4spin(Ham,io,lat)
    ...
    if(io%is_set)then
        ...
        do i_4spin=1,N4spin
            bnd=[sum(pair_dat(:i_4spin-1)%Npair)+1,sum(pair_dat(:i_4spin)%Npair)]
...
subroutine set_mode_pair(Ncell,Ncon,lat,at_mag,pos_offset,col)
