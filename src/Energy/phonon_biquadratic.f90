...
use m_input_H_types, only: io_H_Ph_Biq
use m_io_utils, only: get_parameter,get_coeff,number_nonzero_coeff,max_ind_variable
implicit none
character(len=*),parameter  :: ham_desc="Biquadratic phonon exchange"
private
public :: get_Ph_Biq, read_Ph_Biq_input
contains

subroutine read_Ph_Biq_input(io_param,fname,io)
    integer,intent(in)              :: io_param
    character(len=*), intent(in)    :: fname
    type(io_H_Ph_Biq),intent(out)  :: io

    Call get_parameter(io_param,fname,'U_biq',io%pair,io%is_set)
end subroutine

subroutine get_Ph_Biq(Ham,io,lat)
    ...
    if(io%is_set)then
        ...
        Nph=lat%nph
        allocate(Htmp(Nph*9,Nph*9))!local Hamiltonian modified for each shell/neighbor
        do i_atpair=1,N_atpair
            ...
            do i_dist=1,N_dist
                ...
                do i_shell=1,neigh%Nshell(i_dist)
                    ...
                    ind_ham(:,1)=[((atind_ph(1)-1)*3+i,i=1,9)]
                    ind_ham(:,2)=[((atind_ph(2)-1)*3+i,i=1,9)]
                    Htmp=0.0d0
                    do i=1,9
                    ...
                    Call Ham_tmp%init_mult_connect_2(neigh%pairs(:,connect_bnd(1):connect_bnd(2)),val_tmp,ind_tmp,"UU","UU",lat,2,[9*Nph,9*Nph])
        ...
        dim_mat=[Nph*9*lat%Ncell,lat%U%dim_mode*lat%Ncell]
        nnz=dim_mat(1)
        ...
        do i=0,lat%ncell-1
            do j=0,Nph-1
                col(i*Nph*9+j*3+1)=i*Nph*3+j*3+1
                col(i*Nph*9+j*3+2)=i*Nph*3+j*3+2
                col(i*Nph*9+j*3+3)=i*Nph*3+j*3+3
                col(i*Nph*9+j*3+4)=i*Nph*3+j*3+1
                col(i*Nph*9+j*3+5)=i*Nph*3+j*3+2
                col(i*Nph*9+j*3+6)=i*Nph*3+j*3+3
                col(i*Nph*9+j*3+7)=i*Nph*3+j*3+1
                col(i*Nph*9+j*3+8)=i*Nph*3+j*3+2
                col(i*Nph*9+j*3+9)=i*Nph*3+j*3+3
                ii=ii+9
            enddo
        enddo
        ...
        do i=0,lat%ncell-1
            do j=0,Nph-1
                col(i*Nph*9+j*3+1)=i*Nph*3+j*3+1
                col(i*Nph*9+j*3+2)=i*Nph*3+j*3+1
                col(i*Nph*9+j*3+3)=i*Nph*3+j*3+1
                col(i*Nph*9+j*3+4)=i*Nph*3+j*3+2
                col(i*Nph*9+j*3+5)=i*Nph*3+j*3+2
                col(i*Nph*9+j*3+6)=i*Nph*3+j*3+2
                col(i*Nph*9+j*3+7)=i*Nph*3+j*3+3
                col(i*Nph*9+j*3+8)=i*Nph*3+j*3+3
                col(i*Nph*9+j*3+9)=i*Nph*3+j*3+3
                ii=ii+9
            enddo
        enddo
