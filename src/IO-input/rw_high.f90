...
subroutine read_file(this,prefix,io,fname)
    use m_convert
    use m_io_read_util
    use m_io_utils
    class(parameters_IO_highs),intent(inout) :: this
    ...
    call get_parameter(io,fname,prefix//'_N_highsym',this%N_highsym)
    ...
    call get_parameter(io,fname,prefix//'_highs_frac',this%N_highsym,this%k_highs_frac)
    !read high symmetry positions
    Call set_pos_entry(io,fname,prefix//'_highs_pts',success)
    read (io,*)
    do i=1,this%N_highsym
        read (io,'(a)',iostat=stat) string_tmp
        if (stat /= 0) ERROR STOP "Failed to read line after .._highs_pts, check input"
        ...
        if(stat/=0) ERROR STOP "Failed to read .._highs_pts input, no 3 reals?"
    enddo
    call get_parameter(io,fname,prefix//'_highs_dist',this%aim_dist)
end subroutine
...
end module m_parameters_rw_high
