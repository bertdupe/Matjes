...
!calculates the effective internal magnetic field acting on the magnetization for the dynamics
    ...
    do i=1,size(Ham)
        ...
        if(Nl==1.and.Nr==1.and.Nl_order==1.and.Nr_order==1)then
            !only works if Hamiltonian in constructed fully symmetric, but then it avoids doing the extra mult_l
        ...
        !One might even get away with only doing one mult_*_red if both sides has the Ham_type
