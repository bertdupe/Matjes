...
!internal
      ...
      real(kind=8) :: height,tipx,tipy,r,p_eff,tipz
      real(kind=8) :: kappa,cosinus,cos_theta,vec_x1,vec_x2,vec_y1,vec_y2,cut
      integer :: x_units,y_units,n_spins,maximal
      real(kind=8), dimension(:,:), allocatable :: spins
      real(kind=8) :: tamr,gamm,x_length,y_length,verh
      character(len=100) :: datei,ratio,si1,si2,si,xlength,ylength
      character(len=100) :: bound,blaaa
      character(len=3) :: nummer
      character :: komma
      character(len=10**5) :: string,punkte
      real(kind=8) :: max_z,min_z,abs_z,vec_len,latt,lx,ly,lz,dir1(3),dir2(3)
      real(kind=8) :: orbital,ys,ye,xs,xe,betr
      real(kind=8), dimension(:), allocatable :: dat,speicher1
      real(kind=8) :: cos_phi,he,net(3,3)
      integer :: orb_choice,dim_lat(3)
      integer, dimension(7) :: elemente
      integer, dimension(2) :: nn
      character(len=8) :: xposi,yposi,zpo
      real(kind=8) :: zposi,povscale,yscan,xscan
      real(kind=8) :: xmed,ymed,dx,dy,vec_test(2),vec_norm,cutdown
      logical :: i_dir,i_old,i_user_spin
!     colors
      real(kind=8) :: phi_color
...
! Definition der Rahmenbedingungen aus 'config.dat'
      ...
      xmax=2**int(config(6))
      ymax=2**int(config(7))! Auflösung des betrachteten Bereichs ergibt sich zu xmax*ymax
      ...
      x_units=int(config(12)) ! Anzahl der auszugebenden Bildausschnitte in x-Richtung
      y_units=int(config(13)) ! """""  y-Richtung
      ...
      orb_choice=int(config(21)) ! Auswahl des Tip-Orbitals
